  ...
  return (
    <header className="relative flex justify-between items-center px-4 py-3 bg-white dark:bg-blackish text-grey dark:text-lavender shadow-md">
      <div className="flex items-center gap-2 px-[2rem]">
        ...
        <p className="text-[3.6rem] text-blueViolet dark:text-brightLavender">
          Blorp
        </p>
      </div>
      ...
      <button onClick={toggleTheme} className="text-[3.6rem] px-[2rem]">
        {theme === "dark" ? "☀️" : "🌙"}
      </button>
      <div className="flex gap-2 md:hidden">
        <button onClick={handleToggleMenu} className="text-[3.6rem]">
          {menuOpen ? "✖" : "☰"}
        </button>
      </div>
